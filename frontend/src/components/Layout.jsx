...
import { GraduationCap, LogOut, LayoutDashboard, Target, Bell, Info, User, MessageCircle, Menu, X, ShieldCheck, UserCheck, Rocket, Brain } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#05050C] text-slate-200 flex flex-col font-['Outfit'] relative overflow-hidden">
      ...
      <nav className="fixed top-6 left-1/2 -translate-x-1/2 z-50 w-[95%] max-w-6xl bg-white/[0.03] backdrop-blur-3xl border border-white/10 rounded-full h-16 flex items-center justify-between px-6 shadow-[0_10px_40px_rgba(0,0,0,0.5)]">
        <div className="flex items-center gap-6">
          ...
          <div className="hidden md:flex items-center gap-2 ml-4">
            ...
            <NavLink to="/quiz" className={({isActive}) => `px-4 py-2 rounded-full text-sm font-bold flex items-center gap-2 transition-all ${isActive ? 'bg-indigo-600 text-white shadow-[0_0_20px_rgba(99,102,241,0.4)]' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}><Brain size={16}/> Quiz</NavLink>
      ...
      {showMobileMenu && (
        <div className="md:hidden fixed top-24 left-1/2 -translate-x-1/2 w-[95%] bg-black/80 backdrop-blur-3xl border border-white/10 rounded-3xl p-4 shadow-[0_10px_40px_rgba(0,0,0,0.8)] z-40 flex flex-col gap-2">
            ...
            <NavLink to="/quiz" onClick={() => setShowMobileMenu(false)} className={({isActive}) => `p-3 rounded-xl text-sm font-bold flex items-center gap-3 transition-all ${isActive ? 'bg-indigo-600 text-white shadow-[0_0_20px_rgba(99,102,241,0.4)]' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}><Brain size={18}/> Quiz</NavLink>
