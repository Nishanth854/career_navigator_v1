...
import { GraduationCap, LogOut, LayoutDashboard, Target, Bell, Info, User, MessageCircle } from 'lucide-react';
      ...
      <nav className="fixed top-6 left-1/2 -translate-x-1/2 z-50 w-[95%] max-w-6xl bg-white/[0.03] backdrop-blur-3xl border border-white/10 rounded-full h-16 flex items-center justify-between px-6 shadow-[0_10px_40px_rgba(0,0,0,0.5)]">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative" ref={chatRef}>
            <button onClick={() => setShowChat(!showChat)} className={`p-2.5 rounded-full transition-all flex items-center justify-center border ${showChat ? 'bg-indigo-600 border-indigo-500 text-white shadow-[0_0_20px_rgba(99,102,241,0.6)]' : 'bg-black/40 border-white/10 hover:bg-white/10 text-slate-300'}`}>
          ...
          <div className="relative">
            <button onClick={() => setShowProfile(!showProfile)} className="flex items-center gap-2 bg-black/40 border border-white/10 p-1.5 rounded-full pr-4 hover:bg-white/10 transition-all">
              <div className="w-8 h-8 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-full flex items-center justify-center text-[10px] font-black text-white shadow-lg">{displayName[0].toUpperCase()}</div>
